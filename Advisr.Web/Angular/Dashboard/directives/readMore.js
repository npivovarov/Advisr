...
angular.module('DashboardApp').directive('readMore', function () {
    return {
        restrict: 'A',
        transclude: true,
        replace: true,
        ...
        link: function (scope, elem, attr, ctrl, transclude) {
            ...
            function readmore(text) {
                ...
                elem.append(markup);
                 $(elem[0]).find('.read-more').on('click', function () {
                    $(this).hide();
                    $(elem[0]).find('.more-text').addClass('show').slideDown();
                });
                $(elem[0]).find('.read-less').on('click', function () {
                    $(elem[0]).find('.read-more').show();
                   $(elem[0]).find('.more-text').hide().removeClass('show');
                });
